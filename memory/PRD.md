...
- **Deployment files cleanup** (Feb 2026) — Removed all references to the deleted `whatsapp-service` from CI/CD: deleted `Dockerfile.whatsapp`, removed `letsm-whatsapp` build step from `.github/workflows/deploy.yml`, removed `whatsapp` service block (+ `wa_auth` volume + `WHATSAPP_SERVICE_URL` env) from `docker-compose.prod.yml` and `docker-compose.deploy.yml`. GH Actions pipeline now builds cleanly. (Note: backend `routes/admin.py` and `routes/services.py` still hold legacy `WHATSAPP_SERVICE_URL` references — dead code paths, harmless, can be cleaned later.)
