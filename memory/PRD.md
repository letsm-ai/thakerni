...
- **Audit Logs filters + CSV export** (Jun 3) — Added action dropdown, free-text search (target email / actor / target id), from/to date range, and Clear Filters button on `/admin/audit`. New endpoint `GET /api/admin/audit-logs/export` streams a CSV (timestamp, action, actor, target, plan-from/to, cycle, expires, details) honouring the same filters. Tested 10/10 pass.
- **Team Shared Calendar tab** (Jun 3) — New tab in `/dashboard/team` between Reminders and Chat. Monthly grid colour-coded per team member, prev/next/today nav, member filter, click-day-for-detail panel. Backed by `GET /api/teams/calendar?from_date=&to_date=` returning unified `events` (team tasks with due_date + team reminders), `members` (legend), and `counts`.
- **Landing.js refactor** (Jun 3) — Extracted ~100 lines of static data arrays (features, conversations, testimonials, FAQs, plans, integrations) into `/app/frontend/src/data/landingData.js`. Landing.js: 654 → 552 lines. (Regression fix applied: missed icon imports `PaperPlaneTilt`, `Calendar`, `ChatTeardrop` added back after testing caught a white-screen bug.)
