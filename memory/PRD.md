...
- **WhatsApp Cloud API admin panel** (Jun 4) — New `/admin/whatsapp` page lets the admin manage the AI assistant's WhatsApp number directly via Meta Cloud API (no phone QR needed). Features: configuration status (4 env vars), auto-reply toggle, test send button, recent messages table with inbound/outbound stats, full setup guide inline. Incoming messages are now auto-replied by **gpt-4o-mini** in the same language as the user (Arabic ↔ Arabic, English ↔ English). New backend endpoints: `GET /api/whatsapp/cloud/settings`, `PUT /api/whatsapp/cloud/settings`, `GET /api/whatsapp/cloud/messages`. Setup guide at `/app/memory/WHATSAPP_CLOUD_SETUP.md`. **Required env vars on VPS**: `META_WEBHOOK_VERIFY_TOKEN`, `META_APP_SECRET`, `META_PERMANENT_ACCESS_TOKEN`, `META_PHONE_NUMBER_ID`.
