...
- **Sentry error monitoring** (Jun 4) — Frontend (`@sentry/react`) wraps the entire App in `Sentry.ErrorBoundary` showing a bilingual fallback UI when JS crashes (no more white-screens). Backend uses `sentry-sdk` with FastAPI + Starlette integrations to capture 5xx errors automatically. Both are gated by env vars (`REACT_APP_SENTRY_DSN` / `SENTRY_DSN`) — when unset, SDK init is skipped. Setup guide at `/app/memory/SENTRY_SETUP.md`.
