...
import {
  buildHeroPills, buildFeatures, buildAdvancedFeatures, buildConversations,
  buildDemoScenarios, buildTestimonials, buildPrivacyFeatures, buildFaqs,
  buildMessagingIntegrations, buildCalendarIntegrations, buildPlans,
  HERO_IMAGE, PRIVACY_IMAGE,
...
const Landing = () => {
  ...
  // ── Data (extracted to /data/landingData.js) ──
  const heroPills = buildHeroPills(t);
  const features = buildFeatures(t);
  const advancedFeatures = buildAdvancedFeatures(t);
  const conversations = buildConversations(t);
  ...
  const demoScenarios = buildDemoScenarios(t);
  ...
  // ── Static data (extracted to /data/landingData.js) ──
  const testimonials = buildTestimonials(t);
  const privacyFeatures = buildPrivacyFeatures(t);
  ...
  const messaging = buildMessagingIntegrations(t);
  const calendars = buildCalendarIntegrations();
  const plans = buildPlans(t);
  const heroImage = HERO_IMAGE;
  const privacyImage = PRIVACY_IMAGE;
