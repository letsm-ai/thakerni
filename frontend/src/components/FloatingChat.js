...
import { ChatTeardrop, X, PaperPlaneTilt, Sparkle, ArrowRight } from '@phosphor-icons/react';
...
const FloatingChat = ({ language, isRTL }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        role: 'ai',
        content: language === 'ar'
          ? 'عذراً، حدث خطأ. سجّل للحصول على تجربة كاملة!'
          : 'Sorry, something went wrong. Sign up for the full experience!'
      }]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed bottom-6 right-6 z-50 w-[380px] max-w-[calc(100vw-2rem)] h-[540px] max-h-[calc(100vh-3rem)] bg-white rounded-2xl shadow-2xl shadow-slate-900/20 flex flex-col overflow-hidden border border-slate-200"
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gradient-to-b from-slate-50/80 to-white">
            ...
            {msgCount >= 3 && (
              <div className="bg-gradient-to-r from-violet-50 to-indigo-50 border border-indigo-100 rounded-xl p-3 text-center">
                ...
                <Link
                  to="/login"
                  className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-violet-600 text-white text-xs font-semibold rounded-full hover:bg-violet-700 transition-colors"
